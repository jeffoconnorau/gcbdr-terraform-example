...
unmount_delete_fun(){
  label="$1"
  appid="$2"
  activeimages=`curl -s -f -XGET -H "backupdr-management-session: Actifio $session" -H "Authorization: Bearer $token" -H "Content-Type: application/json" "$MS_URL/actifio/backup?filter=jobclass:==5&filter=jobclass:==15&filter=jobclass:==19&filter=jobclass:==32&filter=jobclass:==48&filter=jobclass:==59&filter=jobclass:==56&filter=jobclass:==52&filter=jobclass:==68&filter=jobclass:==69&filter=characteristic:==1&filter=characteristic:==2&filter=appid:==$appid"`


   echo "$activeimages" > $TMPPATH/test_$label.out
   #mountid=$(jq -r '.items[] | select(.backupname == "'"$backupid"'") | .id' $TMPPATH/test_$label.out)
   mountid_list="$(jq -r '.items[] | select(.label == "'"$label"'") | .id' $TMPPATH/test_$label.out |xargs)"
   if [[ -z "$mountid_list" ]] || [[ "$mountid_list" == "null" ]]; then
      return 0
   fi
   for mountid in ${mountid_list}
   do
     echo " ======= Submitting unmount job ==========="
      unmount=`curl -s -f -XPOST -H "backupdr-management-session: Actifio $session" -H "Authorization: Bearer $token" -H "Content-Type: application/json" "$MS_URL/actifio/backup/$mountid/unmount" -d '{"delete": "true", "force": "true", "label":"'"$label"'"}'`
     sleep 10

     jobs=`curl -s -f -XGET -H "backupdr-management-session: Actifio $session" -H "Authorization: Bearer $token" -H "Content-Type: application/json" "$MS_URL/actifio/jobstatus?filter=label:==$label&filter=jobclass:==unmount-delete&filter=status:==running"`
    ...
    echo "$jobs" > $TMPPATH/data_$label.json  # Save data to a file
     ...
     while [[ -z "$jobid" ]] || [[ "$jobid" == "null" ]]; do
	   ...
	   jobs=`curl -s -f -XGET -H "backupdr-management-session: Actifio $session" -H "Authorization: Bearer $token" -H "Content-Type: application/json" "$MS_URL/actifio/jobstatus?filter=label:==$label&filter=jobclass:==unmount-delete"`
	   echo "$jobs" > $TMPPATH/data_$label.json  # Save data to a file
	   jobid=$(jq -r '.items[0].id' $TMPPATH/data_$label.json)
	   count=$(($count+1))
	   if [[ "$count" -eq 120 ]]; then
	     echo "Unable to get the JOBID"
     ...
     count=0
     while [[ "$jobstatus" == "running" ]] || [[ -z "$jobstatus" ]]; do
	echo "====== Unmount-delete job $jobid is stil running ====="
	sleep 5
	...
	count=$(($count+1))
	if [[ "$count" -gt 80 ]] && [[ -z "$jobstatus" ]]; then
	   echo "ERRORMSG: Unable to get the unmount-delete job status"
	   exit 1
	fi
     done
     if [[ "$jobstatus" == "succeeded" ]]; then
	echo "=== Unmmount job completed succcessfully ==="
	remove_file $TMPPATH/data_$label.json
	remove_file $TMPPATH/test_$label.out
	remove_file $TOUCH_DIR/."$appid"_mount.txt
     else
       echo "ERRORMSG: Failed to run unmoun & delete"
       remove_file $TMPPATH/data_$label.json
       remove_file $TMPPATH/test_$label.out
       exit 1
     fi
...
if [[ -f $TOUCH_DIR/."$appid"_mount.txt ]]; then
   label_list="$(cat $TOUCH_DIR/."$appid"_mount.txt | xargs)"
   for label_name in ${label_list}
   do
    unmount_delete_fun "$label_name" "$appid"
    retval=$?
    if [[ "$retval" -gt 0 ]]; then
       echo "ERRORMSG: Failed to submit the unmount - delete "
    fi
   done
fi
...
if [[ "$jobstatus" == "succeeded" ]]; then
   ...
   #unmount phase
   unmount_delete_fun "$label" "$appid"
   if [[ "$?" -eq 0 ]] ; then
      echo "========== Unmount delete completed successfully ======"
   fi
else
